...
SonarCloud is a hard quality and deployment gate. The project must have zero open SonarCloud issues at `https://sonarcloud.io/project/issues?issueStatuses=OPEN&id=Willseed_debtlab` and zero unreviewed Security Hotspots; any open issue or unreviewed hotspot there is not allowed. Do not report work as complete, merge, or deploy while SonarCloud has open issues or Security Hotspots. Fix issues in code or valid configuration first; do not silence, ignore, downgrade, or bypass SonarCloud findings unless the rule is demonstrably false-positive and the exception is explicit, narrow, documented, and approved. SonarCloud analysis may run through SonarCloud Automatic Analysis or CI, but CI and deployment workflows must keep the open-issues and Security Hotspots checks before any GitHub Pages or Cloudflare Worker deployment.
